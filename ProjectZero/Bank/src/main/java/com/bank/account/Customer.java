...
public class Customer{
	...
	public ArrayList<Transfers>getpending(int index) {
		...
		ArrayList<Transfers> transList = tempList.get(index).getPendingTransfer(cus);
		ArrayList<Transfers> tempList = new ArrayList<Transfers>();
		for(int i = 0; i< transList.size(); i++) {
			if(transList.get(i).getRecievingAcc().equalsIgnoreCase(this.tempList.get(index).getAccNumber()) && transList.get(i).getState().equalsIgnoreCase("P")) { 
